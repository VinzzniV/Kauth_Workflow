...
| S7-Slice1.2 | **Snapshot-Records definieren** (Input fuer `Engine.Plan`): `WorkflowGraphSnapshot`, `WorkflowRuntimeStateSnapshot`, `WorkflowAnswerSnapshot`, `WorkflowContextSnapshot` (inkl. `ApprovalSpecByNodeId` aus Q6/Slice 0). Eigene Datei `WorkflowRuntimeEngineSnapshot.cs`. Reine Records, noch ohne Konsumenten. | LOW | 0,3 d | low | sonnet | pending |
| S7-Slice1.3 | **Plan-Records definieren** (Output von `Engine.Plan`): `RuntimePlan`, `AutoCompleteStep`, `DecisionStep`, `SupervisorApprovalBridgeSkipStep`, `MeasureNodeActivationStep`, `WaitNodeActivationStep`, `WorkflowCompletionStep`, `WorkflowFailureStep`, `WorkflowWaitStep`, plus `ApplyResult { ImmediatelyCompletedMeasureNode }` fuer Q6. Eigene Datei `WorkflowRuntimePlan.cs`. Reine Records. | LOW | 0,3 d | low | sonnet | pending |
| S7-Slice1.4 | **`Engine.Plan(...)` schreiben**: pure Methode, die einen `WorkflowRuntimePlan` aus Snapshot + completedNode ableitet. Spiegelt die Logik aus `AdvanceRuntimeUntilWaitOrTerminal` 1:1, aber ohne DB-Calls. Wirft kein, signalisiert Failure als Plan-Variant. Noch nicht im Loop verwendet. | HIGH | 1,5–2 d | high | opus | pending |
| S7-Slice1.5 | **Repo-Adapter `LoadRuntimeSnapshot` + `ApplyRuntimePlan`**: Snapshot in einem Read-Block laden (alle DB-Reads aus dem Loop konsolidiert); Plan-Apply schreibt INSERTs/UPDATEs + Tasks/Automation-Jobs. `ApplyResult.ImmediatelyCompletedMeasureNode` wird aus `EnsureRuntimeSetupTasksGenerated`-Output abgeleitet. | HIGH | 1–1,5 d | high | opus | pending |
