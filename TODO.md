...
| Z7-3 | `WorkflowDefinitionValidationService.cs` (2131 Z.) splitten in `WorkflowDefinitionDraftValidator` + `WorkflowDefinitionSnapshotValidator` + `WorkflowDefinitionValidationHelpers` + `WorkflowDefinitionValidationCatalog` (Konstanten). Bestehende Tests anpassen. | MEDIUM | in Arbeit | medium | sonnet | Slice-Plan in `CODE_REVIEW.md` § Z7-3. |
...
| Z7-3.4 | `WorkflowDefinitionDraftValidator` extrahieren (`NormalizeDefinitionKey`, `ValidateAndNormalize` + Draft-Overload-Varianten); Service als duenne Facade oder entfernen. | MEDIUM | offen | medium | sonnet | nach Z7-3.3 |

**Naechster sinnvoller Schritt:** Z7-3.4 (`WorkflowDefinitionDraftValidator` extrahieren). Reasoning: medium. Modell: sonnet.
