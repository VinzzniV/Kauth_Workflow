...
**Resthebel ab jetzt:** `CreateErpEmployee` als eigener Backend-Architektur-Slice (Plan-Mode-Slice mit ERP-System-Auswahl). Optional: Workflow-Engine-Branch-on-automation-output fuer AlreadyExists-Faelle (Schritt-6-offene-Folge); `RemoveMailboxLicense` (User-Deprovisionierung); Lizenz-Pool-Monitoring; Sub-License-Disabling; Connection-Pooling im LdapsAdUserWriter/LdapsAdGroupMembershipWriter; Vault-Cleanup-Sweeper; Key-Rotation; Builder-UI-Vereinfachung fuer die `created_ad_user`/`created_mailbox`-Verkettungen.
