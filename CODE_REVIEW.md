...
**Admin-Gated-Automation Slices 1..7 + AGA-N2 + Offboarding- + Change-Handler (2026-05-13 .. 2026-05-19)** — Plan-Vorschau (WhatIf), Task-Automation-Binding, Approval-Endpoint mit echtem Entra-Re-Auth (MSAL `prompt: 'login'` + `auth_time`-Claim-Check, AGA-N2 Commit `6a7c7bb`), Builder-UI, Approval-Runtime-Dialog mit Bundle-Stepper, Live-Log mit per-Action-Status, 360°-Karte-Aggregator (Commit `5fcd388`), Referenzuser-Mapping `reference_user.groups` via Graph App-only (Commit `102ae3a`), Offboarding-Handler `DisableAdUserLdaps` + `RemoveFromAllGroupsLdaps` + `RemoveMailboxLicense` (Commit `0b9d225`), Change-Handler `MoveAdUserOuLdaps` + `UpdateAdUserAttributesLdaps` (neun produktive Handler total). Detail-Historie in `CODE_REVIEW_ARCHIVE.md` § „Admin-Gated-Automation Slices 1-7".

**Optionale offene Resthebel (kein blocker):** `RenameAdUserLdaps` (heikel: UPN-Wechsel hat Entra/Exchange-Folgen, eigener Slice mit Opus + Zwingend Plan-Mode); Builder-UI-Erweiterung fuer `automation_output`-Bedingungen; Lizenz-Pool-Monitoring; Connection-Pooling LDAPS; Vault-Cleanup-Sweeper; Key-Rotation.
