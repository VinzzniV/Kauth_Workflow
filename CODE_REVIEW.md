...
| Z12-2.2 | Frontend Admin-Dashboard-Betriebsblock: Erweiterung des bestehenden `admin-health-panel` um die neuen Runtime-Signale; Severity-Anzeige gemaess Z12-1.2; keine konkurrierende zweite Betriebslogik | HIGH | medium..high | sonnet | done (2026-05-06) — `DashboardAdminRuntimeHealthBlock` (neu) als Zone 4 in `DashboardOverview` eingebaut, nur fuer `dashboardPersona === "admin"`; Severity-Badge (`overallSeverity`), API-Prozess-Kachel (Uptime, Managed Heap), Abhaengigkeiten-Kachel (DB/Auth), Storage-Kacheln (nur wenn `storage[]` nicht leer); `useQuery` mit `staleTime 60s`/`refetchInterval 120s`; Wording „Betriebsstatus", kein „Server"/„RAM"/„Disk"; `AdminOverviewWorkspaceSection` unveraendert; `--critical`/`--head-row`/`--severity-badge`/`--meta--runtime` CSS ergaenzt |
