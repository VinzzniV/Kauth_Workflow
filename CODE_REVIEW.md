...
**Etappe 9a Schritt 7 ✓ 2026-05-13** — `CreateMailboxGraph` (Exchange Online via Graph App-only) produktiv. (a) Per-Action-Retry-Override-Spalten (`max_attempts_override`, `subsequent_retry_delay_seconds_override`) auf `action_definitions`. NULL = globaler Default. Erlaubt einzelnen Handlern ein laengeres Retry-Budget, ohne dass alle anderen mit-laufen. `WorkflowAutomationRetryPolicy.EvaluateRetryOutcome` und `WorkflowAutomationRetrySettings.ResolveRetryDelay` lesen die Overrides defensiv (nicht-positive Werte = "nicht gesetzt"). Beide Failure-Pfade (lokal + External) reichen die Werte aus `ClaimedAutomationJobRecord`/`ExternalCompletionContext` durch. (b) `IGraphMailboxProvisioner` + `GraphMailboxProvisioner` mit SMTP-Strictness: nach `assignLicense` macht der Provisioner einen zweiten `GET /users/{id}` und liest die echte SMTP aus `proxyAddresses` (uppercase `SMTP:` = primary) oder `mail` — **kein UPN-Fallback**. Wenn weder noch da: `MailboxProvisioningInProgress`-Transient → Retry liest spaeter nach. Discriminated-Union-Outcome mit fuenf Varianten (Provisioned / UserNotInDirectoryYet / MailboxProvisioningInProgress / Permanent / TransientFailure) — UserNotInDirectoryYet und MailboxProvisioningInProgress sind getrennt, damit operative Logs die zwei Wartepunkte (Entra-Connect-Sync vs Exchange-Provisioning) klar unterscheiden. (c) `CreateMailboxGraphHandler` (Action ID 10) parallel zur Simulation `CreateMailbox` (ID 2). Pflicht-Payload: `userPrincipalName` + `skuId` (UUID); Output: `primarySmtpAddress` + `licenseSkuId` + `assignedAtUtc`. Retry-Override-Werte im Seed: `max_attempts=10` + `subsequent_delay=300s` = 60s + 8×300s = ~41 min Wartezeit-Budget bis zum 10. Versuch — sicher ueber einen 30-min-Sync-Zyklus. (d) `created_ad_user`-Whitelist um `userPrincipalName` erweitert (Pflicht-Spec-Feld; harter Fail bei fehlend/leer); neue enge `created_mailbox`-Source mit Property-Whitelist nur `primarySmtpAddress` (succeeded Attempt MUSS echte SMTP tragen — sonst handler/provisioner contract bug). (e) Pflicht-Permissions: least-privileged `User.Read.All` + `LicenseAssignment.ReadWrite.All` (Application). 401/403-Mapping ist endpoint-spezifisch (verschiedene Meldungen fuer GET vs assignLicense), damit Operative gezielt nachschauen. Praktisch: End-to-End Onboarding `CreateAdUserLdaps → CreateMailboxGraph → SendWelcomeMailGraph` laeuft ohne manuellen Mailbox-Setup durch — Welcome-Mail-Adresse kommt aus der echten Provisioning-Antwort (kein Drift zwischen Eintrag und tatsaechlicher Mailbox).

**Etappe 9a Schritt 8 ✓ 2026-05-13** — AlreadyExists-Branch im Workflow-Engine. Decision-Nodes koennen jetzt zusaetzlich zu Formular-Antworten ueber Automation-Output ihres direkten Predecessors verzweigen. (a) `WorkflowRuntimeSnapshot.AutomationOutputsByNodeKey` neu im DB-freien Engine-Eingabezustand; `LoadRuntimeSnapshot` im EngineAdapter fuellt es ueber den bestehenden `LoadCreatedAdUserOutputsForWorkflowInScope`-Helper (Wiederverwendung aus Schritt 7 Sub-D). `Plan(...)` reicht den Output-Dict an allen vier Stellen (`ResolveNextNodes` initial / Bridge-Skip / Auto-Complete / `ResolveDecisionTarget`) durch. (b) JSON-Form: `referenceKind: 'answer' | 'automation_output'`-Discriminator in `workflow_edges.condition_expression`. Bestehende Form ohne `referenceKind` bleibt als `answer`-Default gueltig (Backwards-Compat — kein Migration-Skript noetig). Discriminated-Union-Record `DecisionConditionRecord` (`AnswerBased` / `AutomationOutputBased`). (c) Drei Validation-Schranken erzwingen das "nur direkter Predecessor"-Versprechen technisch, nicht nur per Doku: Property/Operator-Union-Schnellcheck im Parser; Direct-Predecessor-Schranke ueber `IncomingEdgesByTargetNodeId`; Action-Key-Schranke gegen `NodeActionsByNodeId` (Single-Action-Pflicht — Multi-Action-Source ist `ambiguous`). Verletzung produziert `BuildFailurePlan` mit klarer Meldung statt rohem Throw. (d) `AllowedConditionProperties` hartcodiert auf `CreateAdUserLdaps.alreadyExisted`; Operator-Set Boolean-only (`is_true`/`is_false`). Catalog-DTO `AutomationPropertyCatalogSourceDto` um `ConditionProperties` erweitert (`created_ad_user.alreadyExisted`); Drift-Schutz-Test asserted die Synchronitaet Catalog ↔ Engine-Parser. Praktisch: Workflow `CreateAdUserLdaps → DecisionNode → (true) End-Skip-WelcomeMail | (false/Fallback) AssignGroups → SendWelcomeMail` laeuft jetzt sauber durch; AlreadyExists ist kein Permanent-Failure mehr, sondern ein gueltiger Workflow-Pfad.

**Resthebel ab jetzt:** `CreateErpEmployee` als eigener Backend-Architektur-Slice (zurueckgestellt bis Stakeholder-Wunsch; ERP-Ziel waere InforLN). Optional: Builder-UI-Erweiterung fuer `automation_output`-Bedingungen (Source-Dropdown, `ConditionProperties`-Filter); `RemoveMailboxLicense` (User-Deprovisionierung); Lizenz-Pool-Monitoring; Sub-License-Disabling; Connection-Pooling im LdapsAdUserWriter/LdapsAdGroupMembershipWriter; Vault-Cleanup-Sweeper; Key-Rotation; Builder-UI-Vereinfachung fuer die `created_ad_user`/`created_mailbox`-Verkettungen.
