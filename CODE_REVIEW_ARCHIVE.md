...
## Admin-Gated-Automation Slices 1-7 (Plan-Vorschau + Approval-Runtime + Live-Log + 360°-Karte + Referenzuser-Mapping, 2026-05-13 .. 2026-05-18)

Sieben Slices vom WhatIf-Plan ueber Task-Binding und Re-Auth-Gate bis zum Live-Log, plus 360°-Karte-Aggregator und Referenzuser-Mapping. End-Ergebnis: Admin sieht in der Workflow-Detail-Sicht an task-Nodes mit gebuendeltem Action-Plan einen "Plan anzeigen & freigeben"-Button; Klick oeffnet einen Dialog mit fachlich gerendertem Stepper, Drift-Schutz, Live-Status pro Action (Status-Pill, Versuch N/M, Logs) und automatischem Task-Auto-Complete. Nach Abschluss liefert die `PersonWorkflowHistoryPage` einen 360°-Sicht-Tab pro Person (Identitaets-Snapshot, aktueller Gruppen-Stand aus Entra-Sync + Automation-Provenienz, Mailbox-Stand, Workflow-Spur, Vault-Status). Im Builder kann ein `reference_user.groups`-Mapping eine Gruppen-Liste aus einem per `person_lookup`-Form-Antwort gewaehlten Referenzuser ziehen (Cloud-Gruppen via Graph App-only, Pflicht-Permission `Group.Read.All`).
...
| 4 — 360°-Karte-Aggregator (nachgezogen) | `5fcd388` | Backend: `PersonDetailsReadService` aggregiert fuenf Sub-Reads (Identitaets-Snapshot, aktueller Membership-Stand aus `directory_group_members`, Automation-Provenienz aus `automation_jobs.payload_json`, Mailbox aus `created_mailbox`-Output, Workflow-Spur, Initial-Passwort-Vault-Status); Auth via `CanAccessWorkflowOverview` + Abteilungs-Scope (spiegelt `/people/{id}/workflows`, nicht `CanAccessPeopleDirectory`); neuer Endpoint `GET /people/{personId:long}/360-view`. Frontend: neuer Tab „360°-Sicht" in `PersonWorkflowHistoryPage`, lazy-`enabled` via `activeTab === "360-view"`; `Person360ViewSection`-Komponente mit fuenf Cards (Identitaets-Snapshot, Gruppen-Membership doppelt = Aktueller Stand + Workflow-Spur, Mailbox, Workflow-Spur, Vault-Status). Kein neuer DB-Schema-Eingriff. |
| 5 — Referenzuser-Mapping (nachgezogen) | `102ae3a` | Neuer Form-InputType `person_lookup` (gespeichert als `valueNumber` in `workflow_answers`; CHECK-Constraint erweitert via `db/manual/2026-05-18_person_lookup_input_type.sql`); neue Setter-Strecke `setRequirementPerson(requirementId, personId)` durch `useRequirementEditor` → `RequirementsSelection` → `WorkflowRequirementsPanel` → `WorkflowDetailPage` plus separater Pfad in `SupervisorStepPage`; Admin-Authoring (`AdminAnswerDefinitionSection` + Hook) ergaenzt. Neue Mapping-Source `reference_user` mit Whitelist nur `groups` (licenseSkus out-of-scope, weil `CreateMailboxGraph` single-`skuId` erwartet). `IReferenceUserDirectoryReader` + `GraphReferenceUserDirectoryReader` lesen via Graph App-only Users[upn].MemberOf (Pflicht-Permission `Group.Read.All`). Pre-Loader `LoadReferenceUserGroupsForMappingAsync` in `PostgresWorkflowAutomationOperations` bridged async-Graph-Read in sync-Resolver; Plan- und Approval-Service rufen den Pre-Loader vor `BuildAutomationJobPayloadAsync`. Builder-Mapping-Vertrag (`workflowBuilderEditorHelpers.MappingSource`) bekommt neue Discriminator-Variante `{ source: "reference_user", property, answerKey }`; Editor zeigt zwei Pflichtfelder (Property-Dropdown + `person_lookup`-Antwort). `PersonLookupInput`-Widget mit debouncter Personen-Suche. |
...
- **`PlannedGroup.Source`-Befuellung** beim Plan-Build aus dem `reference_user.groups`-Pfad — Slice 5 reicht die Gruppen-Liste, der `Source`-Tag bleibt heute leer (eigener Folge-Slice: Plan-Service muss die Herkunft pro Gruppe annotieren).
- **`reference_user.licenseSkus`** als Property — `CreateMailboxGraph` braucht single-`skuId`-UUID, kein List. Folge-Slice braucht entweder neuen Property-Key `primaryLicenseSku` oder Multi-SKU-Mailbox-Handler.
- **On-prem-only-AD-Gruppen** im Referenzuser-Read — Slice 5 ist Graph-only; LDAPS-Worker-Read waere eigener Folge-Slice.
- **`person_lookup` im Create-/Start-Flow** — Slice 5 erweitert nur die Requirements-Edit-Seam (Workflow-Detail + Supervisor-Step). HR pflegt heute den Workflow nicht selbst (`canAccessSupervisorStep` kommt nicht aus HR-Rollen).

**Praktisch:** der End-to-End-Pfad (Admin oeffnet Onboarding-Task → sieht 4-Step-Plan → 1 Klick → Live-Status pro Action mit Versuch-Counter und Logs → Task wird automatisch `done` ODER klarer `failed`-Endzustand) ist nutzbar. Nach Abschluss zeigt die 360°-Sicht der Person den finalen Stand (Identitaets-Snapshot, Gruppen mit Provenienz, Mailbox, Vault-Status). Im Builder kann ein Referenzuser-Mapping eine Gruppen-Liste aus einem im Onboarding gewaehlten Vorlagen-User ziehen. Was fuer den vollen Prod-Rollout fehlt: echtes Entra-Re-Auth, Worker-Output-Vertrag fuer `PlannedGroup.Source` plus Browser-Smoke gegen echtes Entra/AD.
