...
| 9a | Schreibender Automation-Layer (Windows-Worker, AD on-prem führt) | ✓ Sub-Architektur 2026-05-12 (Schritt 1); ✓ Worker-Skeleton 2026-05-12 (Schritt 2); ✓ Erster echter LDAPS-Handler `CreateAdUserLdaps` + DPAPI produktiv 2026-05-12 (Schritt 3); ✓ Härtung (failure_kind-Klassifikation + Linux-Stale-Sweep) 2026-05-12 (Schritt 4); ✓ Vertrags-Plumbing + `AssignGroupsLdaps` + `SendWelcomeMailGraph` 2026-05-12 (Schritt 5); ✓ Temporary-Credentials-Vault (pgcrypto, atomare Tx, Welcome-Mail-Anbindung) 2026-05-13 (Schritt 6); ✓ `CreateMailboxGraph` (Exchange Online via Graph App-only, per-Action-Retry-Override fuer Entra-Connect-Sync) 2026-05-13 (Schritt 7); `CreateErpEmployee` als eigener Folge-Slice |
...
6. **Temporary-Credentials-Vault:** ✓ 2026-05-13 — neue Tabelle `temporary_credentials` mit pgcrypto-Symmetric-Encryption (`pgp_sym_encrypt`/`decrypt`). UNIQUE(workflow_node_instance_id, credential_type) macht den atomaren Schreib-Pfad idempotent. GRANTs eng geschnitten (`kauth_worker` INSERT-only; `kauth_api` SELECT+UPDATE; kein DELETE). `PostgresWorkerJobStore.MarkJobSucceededAsync` schreibt Vault-Insert + `credentialVaultId`-Patch im output_json + Attempt-Update + Job-Success in derselben Tx (`ON CONFLICT DO NOTHING` + SELECT-Fallback fuer Stale-Retry-Idempotenz) — kein Crash-Pfad mit Vault-Waise oder fehlendem Pointer. `CreateAdUserLdapsHandler` reicht das Plain-Passwort als `PendingVaultWrite` an den JobStore weiter und packt einen `credentialVaultId=null`-Platzhalter ins Output, den der JobStore patcht. `created_ad_user`-Property-Whitelist erweitert um `credentialVaultId` (UUID, kein Geheimnis; null-Pfad durchgereicht fuer AlreadyExists-Case). `SendWelcomeMailGraphHandler` decryptet zur Run-time via `ITemporaryCredentialRepository.ReadAdInitialPasswordByVaultIdAsync(uuid)` — kein anderer Lese-Pfad existiert. Welcome-Mail-Catalog hat jetzt `{{temporary_password}}`-Placeholder. Vault-Schluessel auf Worker analog DPAPI-Pattern (`vault.config.dpapi`, `install-vault-key.ps1`); Linux-API via `KAUTH_VAULT_KEY`-Env-Var. Praktisch: das Initial-Passwort liegt weder in `automation_jobs.payload_json` noch in `automation_job_attempts.output_json` — End-to-End Onboarding mit echtem Passwort in der Welcome-Mail ist produktionsreif (Fresh-Create-Pfad).
7. **CreateMailboxGraph (Exchange Online via Graph App-only):** ✓ 2026-05-13 — per-Action-Retry-Override-Spalten (`max_attempts_override`, `subsequent_retry_delay_seconds_override`) auf `action_definitions`. NULL = globaler Default greift; `WorkflowAutomationRetryPolicy.EvaluateRetryOutcome` liest defensiv (nicht-positive Werte = "nicht gesetzt"). `IGraphMailboxProvisioner` + `GraphMailboxProvisioner` mit SMTP-Strictness: nach `assignLicense` zweiter `GET /users/{id}` fuer die echte primary SMTP aus `proxyAddresses` (uppercase `SMTP:`) oder `mail` — **kein UPN-Fallback**. Wenn beide leer: `MailboxProvisioningInProgress`-Transient. Outcome-DU mit fuenf Varianten (Provisioned/UserNotInDirectoryYet/MailboxProvisioningInProgress/Permanent/Transient) — Sync-Lag vs Provisioning-Lag getrennt fuer operative Diagnose. `CreateMailboxGraphHandler` (Action ID 10) parallel zur Simulation (ID 2); Pflicht-Payload `userPrincipalName`+`skuId`; Retry-Override 10/300s = ~41 min Wartezeit-Budget bis zum 10. Versuch. `created_ad_user`-Whitelist um `userPrincipalName` erweitert; neue `created_mailbox`-Source mit Property-Whitelist nur `primarySmtpAddress`. Pflicht-Permissions: `User.Read.All` + `LicenseAssignment.ReadWrite.All` (least-privileged Application). End-to-End-Onboarding-Kette `CreateAdUserLdaps → CreateMailboxGraph → SendWelcomeMailGraph` laeuft ohne manuellen Mailbox-Setup durch.
8. **AlreadyExists-Branch im Workflow-Engine:** ✓ 2026-05-13 — Decision-Nodes koennen jetzt zusaetzlich zu Formular-Antworten ueber den Automation-Output ihres direkten Predecessors verzweigen. `WorkflowRuntimeSnapshot.AutomationOutputsByNodeKey` wird im EngineAdapter ueber den bestehenden `LoadCreatedAdUserOutputsForWorkflowInScope`-Helper befuellt; `Plan(...)` reicht es an allen vier Stellen (`ResolveNextNodes` initial + Bridge-Skip + Auto-Complete + `ResolveDecisionTarget`) durch. JSON-Form: `referenceKind: 'answer' | 'automation_output'`-Discriminator in `workflow_edges.condition_expression`; Bestandsform ohne Discriminator bleibt als `answer`-Default gueltig (Backwards-Compat, kein Migration-Skript). Drei Validation-Schranken erzwingen das "nur direkter Predecessor"-Versprechen technisch: Property/Operator-Union-Schnellcheck im Parser; Direct-Predecessor-Schranke ueber `IncomingEdgesByTargetNodeId`; Action-Key-Schranke gegen `NodeActionsByNodeId` (Single-Action-Pflicht — Multi-Action-Source ist `ambiguous`). Verletzung produziert `BuildFailurePlan` mit klarer Meldung. Initiale Whitelist: `CreateAdUserLdaps.alreadyExisted`; Operator-Set Boolean-only (`is_true`/`is_false`). Catalog-DTO um `ConditionProperties` erweitert mit Drift-Schutz-Test gegen die Engine-Map. Praktisch: Workflow `CreateAdUserLdaps → DecisionNode → (true) End-Skip | (false/Fallback) AssignGroups → SendWelcomeMail` laeuft jetzt sauber durch; AlreadyExists ist kein Permanent-Failure mehr.
9. **Folge-Slices (offen):** `CreateErpEmployee` (InforLN; zurueckgestellt bis Stakeholder-Wunsch); Vault-Cleanup-Sweeper (TTL-getriebener Delete-Job); Key-Rotation (`key_version`-Spalte + Multi-Decrypt-Fan-Out); Builder-UI-Erweiterung fuer `automation_output`-Bedingungen (Source-Dropdown, `ConditionProperties`-Filter); `RemoveMailboxLicense` (User-Deprovisionierung); Lizenz-Pool-Monitoring; Sub-License-Disabling; optional Connection-Pooling im LDAPS-Pfad; Builder-UI-Vereinfachung fuer `created_ad_user`/`created_mailbox`-Verkettungen.
