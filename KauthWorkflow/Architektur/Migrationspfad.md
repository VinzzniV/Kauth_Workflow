...
| 9a | Schreibender Automation-Layer (Windows-Worker, AD on-prem führt) | ✓ Sub-Architektur 2026-05-12 (Schritt 1); ✓ Worker-Skeleton 2026-05-12 (Schritt 2); ✓ Erster echter LDAPS-Handler `CreateAdUserLdaps` + DPAPI produktiv 2026-05-12 (Schritt 3); ✓ Härtung (failure_kind-Klassifikation + Linux-Stale-Sweep) 2026-05-12 (Schritt 4); Schritt 5 (Vault + AssignGroups + weitere Handler) offen |
...
5. **Schritt 5 (offen):** Temporary-Credentials-Vault statt Klartext-Passwort im `output_json`, `AssignGroups` LDAPS-Handler (Wiederverwendung des Schritt-3-Layerings), `SendWelcomeMail` real (Linux-side, baut auf `GraphWorkflowEmailNotificationSender` auf), `CreateMailbox` + `CreateErpEmployee` (jeweils eigene Backend-Architektur-Slices). Action-Katalog enthält am Ende keinen `simulated_*`-Eintrag mehr; die `IsSimulated`-Banner aus Z21-S1 verschwinden naturgemäß.
