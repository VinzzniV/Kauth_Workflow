...
8. **Folge-Slices (offen):** `CreateErpEmployee` (ERP-System-Auswahl mit Stakeholder); Vault-Cleanup-Sweeper (TTL-getriebener Delete-Job); Key-Rotation (`key_version`-Spalte + Multi-Decrypt-Fan-Out); Workflow-Engine-Branch-on-automation-output fuer AlreadyExists-Faelle; `RemoveMailboxLicense` (User-Deprovisionierung); Lizenz-Pool-Monitoring; Sub-License-Disabling; optional Connection-Pooling im LDAPS-Pfad; Builder-UI-Vereinfachung fuer `created_ad_user`/`created_mailbox`-Verkettungen.
