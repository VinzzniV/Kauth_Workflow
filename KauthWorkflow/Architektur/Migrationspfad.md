...
7. **Folge-Slices (offen):** `CreateMailbox` (Graph-Mail-Provisioning, eigener Stakeholder-Slice fuer Lizenz-/Quota-Strategie); `CreateErpEmployee` (ERP-System-Auswahl); Vault-Cleanup-Sweeper (TTL-getriebener Delete-Job); Key-Rotation (`key_version`-Spalte + Multi-Decrypt-Fan-Out); Workflow-Engine-Branch-on-automation-output fuer AlreadyExists-Faelle; optional Connection-Pooling im LDAPS-Pfad; Builder-UI-Vereinfachung fuer `created_ad_user.credentialVaultId`-Verkettung.
