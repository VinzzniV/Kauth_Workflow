...
**Schritt 7:** ✓ 2026-05-13 — `CreateMailboxGraph` produktiv. Entscheidungen: (a) Exchange-Online-Lizenz-Assignment via Graph App-only (Hybrid-AD-Annahme: User wird via Entra Connect synchronisiert; Mailbox entsteht automatisch beim Lizenz-Assignment). Lizenz-Quelle: Pflicht-Payload-Feld `skuId` (GUID), pro Workflow-Node konfigurierbar. (b) **Per-Action-Retry-Override** als architektonische Erweiterung der Retry-Policy: neue Spalten `max_attempts_override` + `subsequent_retry_delay_seconds_override` auf `action_definitions`; NULL = globaler Default. Erlaubt langlaufenden Handlern wie CreateMailboxGraph (Entra-Connect-Sync-Lag ~30 min) ein laengeres Retry-Budget (10/300s = ~41 min Wartezeit), ohne dass alle anderen Handler mit-laufen. **`FirstRetryDelay` bleibt global** — erster Check direkt nach kurzer Zeit ist sinnvoll fuer alle Failure-Pfade. (c) **SMTP-Strictness** als kritische Architektur-Entscheidung (Plan-Review): der Handler liefert nur dann Success, wenn Graph eine echte SMTP-Adresse aus `proxyAddresses` oder `mail` liefert — **kein UPN-Fallback** als Success-Output, weil das SendWelcomeMailGraph an eine nicht-existente Mailbox versenden wuerde. Nach `assignLicense` macht der Provisioner einen zweiten `GET /users/{id}`; wenn weder `proxyAddresses` noch `mail` befuellt sind, kommt `MailboxProvisioningInProgress`-Transient zurueck und der Retry liest spaeter nach. (d) **Outcome-DU mit fuenf Varianten** (Provisioned/UserNotInDirectoryYet/MailboxProvisioningInProgress/Permanent/Transient): zwei getrennte Transient-Sub-Cases fuer Sync-Lag vs Provisioning-Lag — operative Diagnose ueber Logs ohne Vermischung. (e) **Verkettung im Hybrid-Onboarding-Workflow:** `created_ad_user`-Whitelist um `userPrincipalName` erweitert (CreateMailboxGraph liest den UPN per Mapping aus dem CreateAdUserLdaps-Output, `directory_identity` greift erst nach Entra-Sync und ist zu spaet); neue `created_mailbox.primarySmtpAddress`-Source fuer die Welcome-Mail-Adresse. **Pflicht-Permissions (least-privileged):** `User.Read.All` (GET) + `LicenseAssignment.ReadWrite.All` (assignLicense). 401/403-Mapping ist endpoint-spezifisch — verschiedene Meldungen fuer GET vs assignLicense. **Bewusst Out-of-Scope:** Lizenz-Pool-Monitoring, Sub-License-Disabling, Lizenz-Removal, Cloud-only-User-Anlage.

**Schritt 8:** ✓ 2026-05-13 — AlreadyExists-Branch im Workflow-Engine. Entscheidungen: (a) **JSON-Discriminator `referenceKind`** in `workflow_edges.condition_expression`: Werte `answer` (Default fuer Backwards-Compat — bestehende Form `{answerKey, operator, ...}` bleibt 1:1 gueltig, kein Migration-Skript) und `automation_output` (neu). Generisch erweiterbar, klarer als ein paralleles Optional-Feld neben `answerKey`. (b) **Andock-Punkt am Snapshot, nicht am Resolver:** neues Feld `WorkflowRuntimeSnapshot.AutomationOutputsByNodeKey` (DB-freier Engine-Eingabezustand) — Output-Daten gehoeren in dieselbe Read-Schicht wie `AnswersByKey`. `LoadRuntimeSnapshot` im EngineAdapter befuellt es ueber den bestehenden `LoadCreatedAdUserOutputsForWorkflowInScope`-Helper; `Plan(...)` reicht es an allen vier internen Stellen durch (`ResolveNextNodes` initial + Bridge-Skip + Auto-Complete + `ResolveDecisionTarget`). (c) **Drei Validation-Schranken** erzwingen den Scope "nur direkter Predecessor" technisch, nicht nur per Doku: Property/Operator-Union-Schnellcheck im Parser (kein Graph noetig); Direct-Predecessor-Schranke ueber `IncomingEdgesByTargetNodeId` im Plan-Lauf; Action-Key-Schranke ueber `NodeActionsByNodeId` mit **Single-Action-Pflicht** (Multi-Action-Predecessor ist `ambiguous` → BuildFailurePlan). Action-Key-Lookup liest aus dem Graph (`NodeActionsByNodeId`), nicht aus der Node-Config — Action-Zuordnung sitzt im Graph. (d) **Zwei getrennte Whitelists**: `AutomationPropertyCatalog.Properties` (Input-Mapping; Bedingungen unzulaessig) und `ConditionProperties` (Bedingungen; Input-Mapping unzulaessig). Damit ist `alreadyExisted` nur in Decision-Conditions referenzierbar, nicht als Input-Mapping-Quelle — getrennte bewusste Entscheidung pro neue Property. Drift-Schutz-Test asserted Catalog ↔ Engine-Map. (e) **Operator-Set Boolean-only** (`is_true`/`is_false`) im ersten Aufschlag. String-/Number-Compares folgen mit dem jeweiligen Use-Case. (f) **Fehlermodell**: Parser-Fehler (Schranke 1) und Plan-Fehler (Schranken 2/3) fliessen beide ueber `Plan(...)`-`try/catch` → `BuildFailurePlan` mit klarer Meldung; kein roher Throw aus dem Parser, der den Runtime-Loop zerlegt. **Bewusst Out-of-Scope:** Builder-UI fuer Output-Bedingungen (Folge-Slice), String/Number-Compares, Loop/History-Tiefe ueber den direkten Predecessor hinaus, Migration bestehender `condition_expression`-Zeilen.

**Nächster Slice:** `CreateErpEmployee` (InforLN; zurueckgestellt bis Stakeholder-Wunsch) als eigener Backend-Architektur-Slice; optional Builder-UI-Erweiterung fuer `automation_output`-Bedingungen, `RemoveMailboxLicense`, Lizenz-Pool-Monitoring, Connection-Pooling, Vault-Cleanup, Key-Rotation.
