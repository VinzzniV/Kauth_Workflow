...
Eroeffnet 2026-05-11 als breiter Read-Vertrags-Folgeblock zu Z10/Z11. S1 ist **reiner Doku-/Planungs-Slice**.

**Praktisch:** Nach Z10/Z11 wurden bewusst sieben Read-/Listen-Vertraege zurueckgestellt (A Identity-Listen, C `/admin/directory/identities`, C Gaps/Pending Split, E Notification-Templates, F Rotation Action-Templates, G Runtime-Sub-Resources, H `/workflow-definitions/startable`). Diese Mitnahmeschnitte sind seither liegen geblieben.
**Lohnenswert:** P1-/P2-Hull-Adapter (`AdminListPage<T>`, `CursorPage<T>`) sind seit Z11 produktiv; ein Bundle-Schnitt loest den Restblock auf, statt jeden Endpunkt als Mikro-Zyklus mit eigenem Doku-Overhead zu fahren.
**Nutzen:** konvergenter Admin-/Directory-/Runtime-Read-Vertrag; Composite-Split fuer C Gaps/Pending in einem kontrollierten Schnitt; FE-Komponenten konvergieren auf zwei Hulls + ein P3-Lookup-Pattern.
...
| Z20-B2 | Block C: `/admin/directory/identities` P1 + Composite-Split Gaps/Pending | HIGH | urspruenglich `claude-opus-4-7` + `--effort high`; umgesetzt durch Codex | **done 2026-05-11** |
| Z20-B3 | Block E (Liste) + F + G: Templates P1 + Runtime-Sub-Resources P2/Cursor | HIGH | urspruenglich `claude-sonnet-4-6` + `--effort medium`; umgesetzt durch Codex | **done 2026-05-11** |
| Z20-B4 | Block A: `/admin/auth/{users,groups,permissions,roles}` + `/admin/directory/unlinked-identities` P1 | MEDIUM | urspruenglich `claude-sonnet-4-6` + `--effort medium`; umgesetzt durch Codex | **done 2026-05-11** |

**Z20-Abschluss:** Die verbliebenen Read-Vertraege aus Z10/Z11 sind geschlossen. Directory-Composite-Reads sind in P1-Listen gesplittet, Settings-/Auth-Listen liefern P1-Huellen, Runtime-Events und Automation-Jobs liefern P2-Cursor-Seiten. Verifiziert mit API-Tests (566 bestanden, 1 uebersprungen) und Frontend-Build.

**Bewusst NICHT in Z20:** Schreibpfade der genannten Endpunkte; breite Architektur-Umbauten am Definition-/Runtime-/Automation-Layer; Berechtigungsmodell-Aenderungen; Mobile-/Tablet-Layout (R10); Z16-S4 Automation-Snapshot-Vertrag (deferred); Z8-3.2/#8 (deferred); `/admin/people` Vertragsaenderung (Z16-S2-Anker); Filterzustand-URL-Persistenz fuer Builder-Tabs (Restgrenze aus Z11-F3); `GET /admin/directory/{status,groups,audit}` (`audit` bereits Z11-F2 P2; `status`/`groups` ohne realen Pagination-Bedarf).

Detail in `CODE_REVIEW.md` § „Abgeschlossener Zyklus 20" (S1-Findings je Block, Reihenfolge-Begruendung, Test-Erwartung, Abschluss).
...
**Praktisch:** HR kann heute nicht direkt zur 360°-Mitarbeiterakte navigieren — es gibt keinen Navigationseinstieg, nur indirekte Links aus Workflows oder dem Rotationsplan. Gleichzeitig ist die Personen-Suche (`/people/search`) an die falsche Permission geknuepft (`CanCreateWorkflow` statt personenbezogener Zugang). **Lohnenswert:** Person ist der fachliche Primaeranker des Systems; einen eigenen Einstieg zu ereroeffnen haelt den Architekturanker sauber und schafft den Andockpunkt fuer spaetere Automationen. **Nutzen:** direkter HR/Admin-Zugang, sauberer Permission-Vertrag, klarer Identity-Snapshot-Vertrag fuer Automationen.

| Befund | Prio | Status |
|--------|------|--------|
| Z16-S1 — Inventur + Vertragsentscheidung | HIGH | done 2026-05-08 — Inventur: kein Nav-Eintrag; `/people/search` auf `CanCreateWorkflow` (falsch); kein `snapshotAt` fuer Automation. Entscheidungen: `peopleDirectory`-Feature + `CanAccessPeopleDirectory`; `/people` Listenseite; Snapshot-Zeitstempel deferred |
| Z16-S2 — BE-Permission-Vertrag: `CanAccessPeopleDirectory` Policy; `/people/search` Fix; `GET /admin/people` P1-Listendpunkt | HIGH | done 2026-05-08 |
| Z16-S3 — FE-Navigation: `peopleDirectory`-Feature + `PeopleDirectoryPage` + Nav-Eintrag HR/Admin + Breadcrumb-Differenzierung | HIGH | done 2026-05-08 |
