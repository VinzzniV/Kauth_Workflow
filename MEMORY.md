...
- **Aktiver Zyklus: Z19 — Backend Full Review / Holistic Audit.** **Z19-S1 + S2 + S3 + S4 + S9/L1 + S9/L4 abgeschlossen 2026-05-11** (S3 + L1 als ein gebuendelter Commit: `db/manual/manifest.json`, `db/manual/README.md`, `api/API.Tests/SchemaParityTests.cs`, Entfernung `db/init/prod;C/`). **Naechster Slice: Z19-S5** — `SystemEventLogService` (UndefinedTable-Swallow ersetzen, Cursor-Pagination, eigene Unit-Tests). Empfehlung Codex-CLI: `--model claude-sonnet-4-6 --effort medium`. Detail in `CODE_REVIEW.md` § „Aktiver Zyklus 19 — S3 + L1 Ergebnis".
...
- **DB-Drift-Pfad:** schemaaendernde Slices erzeugen weiterhin manuelle SQL-Helfer in `db/manual/`. Seit Z19-S3 (2026-05-11) sichert `db/manual/manifest.json` + `SchemaParityTests.cs` ab, dass jede `*.sql`-Datei dort ihren Soll-Endstand in `db/01_schema.sql` widerspiegelt; jede neue Migration muss zusaetzlich zur SQL-Datei einen Manifest-Eintrag mit `expect_in_schema` (+ optional `forbid_in_schema`) bekommen, sonst wird der Test rot. Workflow-Detail: `db/manual/README.md`.
