...
- **Einziger aktiver offener Slice** in `TODO.md`: **Z21-S4 Realer Automation-Pfad**. Etappe 9a Schritt 1 + 2 + 3 ✓ 2026-05-12. Schritt 3 hat den ersten echten LDAPS-Handler `CreateAdUserLdaps` produktiv geliefert (Layering: Core-Handler + Windows-Host-Adapter), DPAPI-Encryption als Default fuer die DB-Konfig (`IDbConfigDecryptor`+`WindowsDpapiDecryptor`), und den gMSA-Service-Switch in `install-windows-service.ps1`. Bewusste Trade-offs dokumentiert: `temporaryPassword` im `output_json` (Vault folgt), Minimal-LDAP-Klassifikation 49/50/32/21/19→Permanent (Generalisierung folgt), `is_idempotent=true` (Klassifikation differenziert folgt). Naechster Code-Slice: **Etappe 9a Schritt 4** — Error-Klassifikation generalisieren, Linux-API Stale-Worker-Claim-Sweep, Temporary-Credentials-Vault, Migration der restlichen `simulated_*`-Handler. Eigener Plan-Mode-Slice vor Start, Aufwand ~3–5 Tage.
