...
- **Naechster Schritt:** Z9-2.3 DB-Sync-Operations-Modul (`IEntraDirectorySyncOperations` + `EntraDirectorySyncOperations`) unter `api/API/Services/Directory/` extrahieren. Methoden gemaess `CODE_REVIEW.md` § Z9-1.2 Punkt 3 (`UpsertDirectoryIdentitiesBatch`/`InsertGroupMembershipsBatch` + Projection/Activation/Sync-Logging). Reflection-Test 177-179 in `PostgresWorkflowRepositoryAdminConfigIntegrationTests.cs` umstellen oder entfernen.
- **Geplante Slices:** Z9-1.1 Inventur (done) → Z9-1.2 Extract-Plan (done) → Z9-2.1 Pre-Cleanup + Phasen-Strukturierung (done 2026-05-05) → Z9-2.2 Graph-Adapter (done 2026-05-05) → Z9-2.3 DB-Sync-Operations-Modul → Z9-3 Coverage (Integration-Tests Batch-Helfer + Unit-Tests Orchestrator gegen Graph-/Ops-Stubs).
