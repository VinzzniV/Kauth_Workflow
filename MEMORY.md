...
- **Stand im Zyklus:** **Z11-F1 abgeschlossen** — `AdminListPageDto<T>` live fuer `/departments`, `/roles`, `/admin/master-data/departments`, `/admin/master-data/positions`, `/admin/master-data/responsibilities`; typed FE-Wrapper vorhanden, aktuelle Consumer lesen pragmatisch `items`. **Z11-F2 abgeschlossen** — `CursorPageDto<T>` + Keyset-Pagination fuer `/admin/auth/audit` und `/admin/directory/audit`; typed FE-Wrapper, Akkumulations-Hook, „Mehr laden"-Knopf in beiden Audit-Tabs. **Naechster Schritt:** **Z11-F3** P1 ausrollen + D Builder-Tabs (sieben scoped Endpunkte) — opus/medium..high.
...
- Z11-F1 und Z11-F2 sind als abgeschlossene FE-Trigger in `FRONTEND_TODO.md` vermerkt; fuer Z11-F3 weiter kein praeventiver Sammeleintrag.
