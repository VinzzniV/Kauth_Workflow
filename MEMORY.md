...
- **Einziger aktiver offener Slice** in `TODO.md`: **Z21-S4 Realer Automation-Pfad**. Etappe 9a Schritt 1 + 2 + 3 + 4 + 5 + 6 ✓ 2026-05-13. Schritt 6 hat den **Temporary-Credentials-Vault** geliefert: (a) Tabelle `temporary_credentials` mit pgcrypto-Symmetric-Encryption (`pgp_sym_encrypt`/`decrypt`); UNIQUE(workflow_node_instance_id, credential_type) macht den atomaren Schreib-Pfad idempotent. (b) Worker-Schreibpfad atomar in `PostgresWorkerJobStore.MarkJobSucceededAsync` (Vault-Insert + Attempt + Job in einer Tx; ON CONFLICT-Recovery; `credentialVaultId`-Patch im output_json). (c) `CreateAdUserLdapsHandler` liefert `PendingVaultWrite` und schreibt nicht mehr `temporaryPassword` plain ins Output. (d) `created_ad_user`-Property-Whitelist um `credentialVaultId` (UUID, kein Geheimnis) erweitert — Null-Pfad wird durchgereicht, kein Sweeper-Loop. (e) `SendWelcomeMailGraphHandler` konsumiert `ITemporaryCredentialRepository.ReadAdInitialPasswordByVaultIdAsync(uuid)` zur Run-time; `{{temporary_password}}`-Placeholder im Catalog-Template. (f) Vault-Key-Loader auf Worker analog DPAPI-Pattern aus Schritt 3 (`KAUTH_WORKER_VAULT_KEY` Env > `vault.config.dpapi` > `vault.config.json`); API-Seite via `KAUTH_VAULT_KEY` Env-Var. Plain-Passwort landet weder in `automation_jobs.payload_json` noch in `automation_job_attempts.output_json`. Naechster Code-Slice: **CreateMailbox** und **CreateErpEmployee** als jeweils eigene Backend-Architektur-Slices (eigener Plan-Mode-Slice).
