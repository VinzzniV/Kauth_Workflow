...
DROP TABLE IF EXISTS rotation_task_comments CASCADE;
DROP TABLE IF EXISTS rotation_task_assignments CASCADE;
DROP TABLE IF EXISTS rotation_notifications CASCADE;
DROP TABLE IF EXISTS rotation_audit_log CASCADE;
DROP TABLE IF EXISTS rotation_generated_tasks CASCADE;
DROP TABLE IF EXISTS department_action_templates CASCADE;
DROP TABLE IF EXISTS rotation_stations CASCADE;
...
CREATE TABLE rotation_stations (
    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE TABLE department_action_templates (
    id INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    department_id INTEGER NOT NULL REFERENCES departments(id) ON DELETE RESTRICT,
...
CREATE TABLE rotation_generated_tasks (
    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    rotation_plan_id BIGINT NOT NULL REFERENCES rotation_plans(id) ON DELETE CASCADE,
...
CREATE TABLE rotation_task_assignments (
    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE TABLE rotation_task_comments (
    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    rotation_generated_task_id BIGINT NOT NULL REFERENCES rotation_generated_tasks(id) ON DELETE CASCADE,
    author_user_id BIGINT REFERENCES app_users(id) ON DELETE SET NULL,
    comment_text TEXT NOT NULL,
...
CREATE TABLE rotation_notifications (
    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    rotation_plan_id BIGINT NOT NULL REFERENCES rotation_plans(id) ON DELETE CASCADE,
    rotation_station_id BIGINT REFERENCES rotation_stations(id) ON DELETE SET NULL,
...
CREATE TABLE rotation_audit_log (
    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    rotation_plan_id BIGINT NOT NULL REFERENCES rotation_plans(id) ON DELETE CASCADE,
    rotation_station_id BIGINT REFERENCES rotation_stations(id) ON DELETE SET NULL,
    generated_task_id BIGINT REFERENCES rotation_generated_tasks(id) ON DELETE SET NULL,
...
CREATE INDEX idx_rotation_plans_person_id ON rotation_plans(person_id);
CREATE INDEX idx_rotation_plans_source_workflow_id ON rotation_plans(source_workflow_id);
CREATE UNIQUE INDEX uq_rotation_plans_active_per_person
    ON rotation_plans(person_id)
    WHERE status = 'active';
CREATE UNIQUE INDEX uq_rotation_plans_open_source_workflow
    ON rotation_plans(source_workflow_id)
    WHERE status IN ('draft', 'active');
CREATE INDEX idx_department_action_templates_department_trigger_active
    ON department_action_templates(department_id, trigger_type, is_active);
CREATE INDEX idx_rotation_generated_tasks_plan_id
    ON rotation_generated_tasks(rotation_plan_id);
CREATE INDEX idx_rotation_generated_tasks_station_id
    ON rotation_generated_tasks(rotation_station_id);
CREATE UNIQUE INDEX uq_rotation_generated_tasks_station_template
    ON rotation_generated_tasks(rotation_station_id, template_id)
    WHERE rotation_station_id IS NOT NULL
      AND template_id IS NOT NULL;
CREATE INDEX idx_rotation_generated_tasks_department_status
    ON rotation_generated_tasks(department_id, status);
CREATE INDEX idx_rotation_generated_tasks_due_date_status
    ON rotation_generated_tasks(due_date, status);
CREATE INDEX idx_rotation_task_assignments_task_id
    ON rotation_task_assignments(rotation_generated_task_id);
CREATE UNIQUE INDEX uq_rotation_task_assignments_primary
    ON rotation_task_assignments(rotation_generated_task_id)
    WHERE is_primary = TRUE;
CREATE INDEX idx_rotation_task_comments_task_id
    ON rotation_task_comments(rotation_generated_task_id);
CREATE INDEX idx_rotation_notifications_plan_type_status
    ON rotation_notifications(rotation_plan_id, notification_type, status);
CREATE INDEX idx_rotation_notifications_station_type
    ON rotation_notifications(rotation_station_id, notification_type);
CREATE INDEX idx_rotation_audit_log_plan_id
    ON rotation_audit_log(rotation_plan_id, created_at DESC);
CREATE INDEX idx_rotation_audit_log_generated_task_id
