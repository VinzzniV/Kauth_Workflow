...
- Seit FE-31 ist `PersonWorkflowHistoryPage` eine 360°-Arbeitsfläche: ein Statuschip-Streifen + Metric-Strip (aktive Vorgänge, offene Aufgaben, ausstehende/fehlgeschlagene Benachrichtigungen) und ein Tab-Strip führen durch vier Bereiche — `Übersicht` (Stammdaten + Verzeichnis-Kontext + Schnellzugriff auf aktive Vorgänge), `Offene Aufgaben` (clientseitig aggregiert, nach SLA/Frist sortiert), `Benachrichtigungen` (fehlgeschlagene + ausstehende Versendungen) und `Vorgänge` (bisherige Karten-/Tabellenansicht der Historie). Die Aggregation läuft über `src/hooks/usePersonWorkflowAggregates.ts`, das `useQueries` nutzt, um für jeden aktiven Vorgang den bestehenden `WorkflowDetail`-Endpoint parallel zu ziehen; der Backend-Vertrag bleibt unverändert.
