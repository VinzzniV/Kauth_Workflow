...
- Seit Z16-S3 gibt es unter `/people` eine eigene Mitarbeiterliste (`PeopleDirectoryPage`) fuer HR + Admin. Die Seite nutzt `GET /admin/people` (paginated, suchbar), zeigt Status-Badge und Verzeichnis-Link-Status und fuehrt direkt in `/people/:personId`. Der Breadcrumb in `PersonWorkflowHistoryPage` differenziert jetzt zwischen Rollen: HR/Admin sieht `/people`, alle anderen sehen `/workflows`.
