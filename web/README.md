...
- Seit FE-31 ist `PersonWorkflowHistoryPage` eine 360°-Arbeitsfläche: ein Statuschip-Streifen + Metric-Strip (aktive Vorgänge, offene Aufgaben, ausstehende/fehlgeschlagene Benachrichtigungen) und ein Tab-Strip führen durch fünf Bereiche — `Übersicht` (Stammdaten + Verzeichnis-Kontext + Schnellzugriff auf aktive Vorgänge), `Offene Aufgaben` (clientseitig aggregiert, nach SLA/Frist sortiert), `Benachrichtigungen` (fehlgeschlagene + ausstehende Versendungen), `Vorgänge` (bisherige Karten-/Tabellenansicht der Historie) und `360°-Sicht` (Identity-Snapshot, Gruppen-/Mailbox-/Workflow-Spur, Vault-Status). Die Aggregation läuft über `src/hooks/usePersonWorkflowAggregates.ts`, das `useQueries` nutzt, um für jeden aktiven Vorgang den bestehenden `WorkflowDetail`-Endpoint parallel zu ziehen; die person-zentrierte 360°-Sicht selbst kommt zusaetzlich aus `GET /people/{id}/360-view`.
- Seit Z16-S3 gibt es unter `/people` eine eigene Mitarbeiterliste (`PeopleDirectoryPage`) fuer HR + Admin. Die Seite nutzt `GET /admin/people` (paginated, suchbar), zeigt Status-Badge und Verzeichnis-Link-Status und fuehrt fuer echte Mitarbeiterkarten direkt in `/people/:personId`. Seit Feature C erscheinen dort zusaetzlich aktive Entra-Identitaeten ohne Mitarbeiterkarte als `directory_only`: kein Detail-Link ohne `personId`, stattdessen Inline-Import pro Karten-/Tabelleneintrag ueber `POST /admin/people/import-from-directory`. Der Breadcrumb in `PersonWorkflowHistoryPage` differenziert jetzt zwischen Rollen: HR/Admin sieht `/people`, alle anderen sehen `/workflows`.
- Seit Z18-S4 ist `/search` ein reiner Legacy-Redirect auf `/workflows` (Query-Params `q`, `dept`, `type`, `status` werden erhalten). `WorkflowSearchPage.tsx` wurde entfernt; alle internen Links und Navigationspunkte zeigen jetzt auf `/workflows`. Die Suche laeuft vollstaendig ueber die paginierte `WorkflowListPage` mit `SavedViews` und echtem Paging.
- Seit Z16-S3 ist `peopleDirectory` ein eigenes `AppFeature` in `roleModel.ts` (HR + Admin); der Navigationseintrag „Mitarbeiter" erscheint nur fuer diese Rollen.
- Seit A2 gibt es unter „Administration → Personen & Zugriff" die Sektion „Aus Entra importieren" (`AdminEntraImportSection`, selbststaendig via useQuery/useMutation). Entra-Mitarbeitende ohne Mitarbeiterkarte werden dort nach Abteilung gruppiert angezeigt, per Checkbox ausgewaehlt und retroaktiv importiert. Deaktivierte Konten sind standardmaessig ausgeblendet. API-Kontrakt: `GET /admin/directory/unlinked-identities` und `POST /admin/people/import-from-directory`.
- Seit FE-12 sind versioned Specs für alle Measure-Nodes (`measure_provision`, `measure_deprovision`, `measure_change`, `measure_rename`) direkt im Builder editierbar. `WorkflowBuilderSpecEditor.tsx` öffnet per Drawer den `node.specs`-Editor: Specs hinzufügen/entfernen, alle Felder bearbeiten, Conditions (answerKey, Operator, expectedValue) und Dependencies (dependsOnSpecKey) verwalten. Der Speicherpfad nutzt den bestehenden Version-Replace-Flow ohne Backend-Vertragsänderung.
- Seit FE-13 schützt der Builder-Save Specs gegen parallele Schreiber: `useAdminWorkflowBuilder.saveVersion` schickt den beim Laden gesehenen `updatedAt`-Stand als `expectedUpdatedAt` an `PUT /admin/config/workflow-definition-versions/{versionId}` mit. Hat ein anderer Schreiber (z. B. der Aufgabenvorlagen-Editor) die Version inzwischen verändert, antwortet das Backend mit `409 Conflict` und einem `WorkflowDefinitionVersionConflictDto`. Der Builder zeigt darauf einen Confirmation-Dialog: „Trotzdem speichern" wiederholt den Replace ohne Token (Force-Overwrite), „Abbrechen und neu laden" lädt den frischen Stand vom Server.
- Seit FE-22 zeigt die Sidebar-Navigation bei „Meine Aufgaben" und „Anforderungen der Abteilungsleitung" rote Badge-Zähler für offene Tasks bzw. wartende Supervisor-Vorgänge. Der Hook `useNavBadgeCounts` (`src/hooks/useNavBadgeCounts.ts`) steuert die Abfragen (eigene Query-Keys, feature-guarded); `AppLayout` rendert `.sidebar-link-badge` in Desktop- und Mobile-Nav. CSS in `base.css`.
- Seit FE-33 zeigt `WorkflowListPage` oberhalb der Filterleiste eine rollenbasierte Ansichten-Leiste (`WorkflowListSavedViewsBar`). Presets wie „HR startet", „Wartet auf Freigabe", „Fachbereiche offen", „In Bearbeitung" und „Abgeschlossen" werden per Rolle ausgewählt (`workflowListSavedViews.ts`): HR sieht alle Phasen, Manager sehen Freigabe-/Bearbeitungsstatus, Worker sehen Fachbereich-relevante Status, Admin/Leser sehen einen Überblicks-Set. Klick auf ein Preset setzt die bestehenden URL-Parameter (`status`, `dept`, `type`, `resp`) atomisch; das aktive Preset wird visuell hervorgehoben. URL-Stabilität ist vollständig gewährleistet, da die existierenden `useSearchParams`-basierten Filter unverändert bleiben.
...
- `peopleApi.ts` enthaelt seit Z16-S3 zusaetzlich `getPeopleDirectory` fuer den `GET /admin/people` Listenpunkt (HR + Admin)
