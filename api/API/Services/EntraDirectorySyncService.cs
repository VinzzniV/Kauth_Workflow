...
internal sealed class EntraDirectorySyncService : IDirectorySyncService
{
    ...
    private readonly IEntraDirectorySyncOperations _operations;
    ...
    public EntraDirectorySyncService(
        ...
        IEntraDirectorySyncOperations operations,
    ...
    {
        await _operations.EnsureDirectoryProjectionUserColumnsAsync(connection, cancellationToken);
        ...
        foreach (var group in matchedGroups)
        {
            ...
            try
            {
                ...
                {
                    var identityIdByEntraId = await _operations.UpsertDirectoryIdentitiesBatchAsync(
                        connection,
                        validUsers,
                        cancellationToken);
                    ...
                    await _operations.InsertGroupMembershipsBatchAsync(
                        connection,
                        directoryGroupId,
                        distinctIdentityIds,
                        cancellationToken);
    ...
    private async Task RunDirectoryProjectionAsync(
        ...
        CancellationToken cancellationToken)
    {
        await _operations.AutoLinkIdentitiesToAppUsersAsync(connection, cancellationToken);

        var departmentSyncResult = await _operations.EnsureDirectoryDepartmentsExistAsync(connection, cancellationToken);
        await _systemEventLogService.WriteAsync(new SystemEventLogWriteModel
        {
            Severity = "info",
            Source = "directory",
        ...
        var appUserProjectionResult = await _operations.UpdateExistingAppUsersFromDirectoryAsync(connection, startedAt, cancellationToken);
        await _systemEventLogService.WriteAsync(new SystemEventLogWriteModel
        {
            Severity = "info",
            Source = "directory",
