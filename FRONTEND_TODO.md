...
**FE-13 Stale-Detection beim Builder-Save 2026-05-04:** Optimistic-Concurrency vollständig umgesetzt. Backend-Vertrag erweitert: `ReplaceWorkflowDefinitionVersionRequest.ExpectedUpdatedAt` (optional, `DateTime?`), Endpoint `PUT /admin/config/workflow-definition-versions/{versionId}` antwortet 409 mit `WorkflowDefinitionVersionConflictDto` (`message`, `currentUpdatedAt`), Repository macht den Vergleich unter `FOR UPDATE` mit ms-Toleranz und wirft `WorkflowDefinitionVersionStaleException`. Frontend (`useAdminWorkflowBuilder.saveVersion`) sendet beim Save den geladenen `updatedAt`-Stand mit; bei 409 erscheint ein Bestätigungs-Dialog: „Trotzdem speichern" wiederholt den Replace ohne Token (Force-Overwrite), „Abbrechen und neu laden" zieht via `getAdminWorkflowDefinitionVersion` den frischen Stand. Endpoint-Test ergänzt (`ReplaceDefinitionVersionEndpoint_ReturnsConflictWhenStale`).
...
**FE-22 Navigations-Badges 2026-05-04:** Sidebar-Navigation zeigt jetzt Badge-Counts für „Meine Aufgaben" (`/tasks/my`) und „Anforderungen der Abteilungsleitung" (`/supervisor`). Neuer Hook `useNavBadgeCounts` (eigene React-Query-Queries mit 60-s-staleTime, feature-guard per `canAccessFeature`). `AppLayout` rendert `.sidebar-link-badge` (rote Pill) für beide Routen; Desktop- und Mobile-Nav pari. `queryKeys.supervisorWorkflows` ergänzt. CSS in `base.css`. Backend-Vertrag unverändert.
