...
| FE-13 | **Stale-Detection beim Builder-Save** (FE-9-Folge). Wenn AdminTaskTemplate-Editor und Builder parallel Specs aendern, kann der Builder-Save die juengere Edit-Generation ueberschreiben. Fix: `updatedAt`-Pruefung beim Replace, 409 Conflict bei Drift, UI fragt zurueck. **Frontend-Schutzmaßnahme ergänzt (2026-05-04):** `saveVersion` macht vor dem Replace einen Preflight-GET und vergleicht `updatedAt`. Bei Drift erscheint ein Bestätigungs-Dialog; „Abbrechen" lädt den Entwurf frisch vom Server. Echter 409 ist nicht möglich — `ReplaceWorkflowDefinitionVersionRequest` hat kein Concurrency-Token. Vollständige Lösung erfordert Backend-Erweiterung. | LOW | 0,5 d | medium | sonnet | offen — Backend-409 fehlt |
